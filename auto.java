...
public class StarterBotAuto extends OpMode
{
    ...
    private enum AutonomousState {
        ...
        Program_ShootAndMove_Init,
        Program_ShootAndMove_DrivingAwayFromGoal, //    }
        Program_ShootAndMove_RotatingAwayFromGoal, //   } These three are reused for Program_MoveShootMove
        Program_ShootAndMove_DrivingOffLine,   //       }

        Program_MoveShootMove_Init,
        Program_MoveShootMove_DrivingToGoal,
        Program_MoveShootMove_RotatingToGoal,
        Program_MoveShootMove_DrivingToShootPosition,
        // goes to Program_ShootAndMove_DrivingAwayFromGoal

        Program_OnlyMove_Init,
        Program_OnlyMove_DriveForward,


        // deprecated (old + not in use)
        DRIVING_AWAY_FROM_GOAL,
        ROTATING,
        DRIVING_OFF_LINE,
        ROTATE2,
        DRIVING_TO_BALLS,
        PUSHING_BALLS_INTO_TEAM_ZONE,
    ...
    private AutonomousState autonomousState = AutonomousState.INIT;
    private AutonomousState nextState = AutonomousState.INIT;

    private double waitTime;
    private ElapsedTime autoStopwatch = new ElapsedTime();
    ...
    private Program program = Program.SHOOT_AND_MOVE;
    ...
    public void init_loop() {
        ...
        if(gamepad1.dpad_up){
            program = Program.SHOOT_AND_MOVE;
        }
        else if(gamepad1.dpad_right){
            program = Program.MOVE_SHOOT_MOVE;
        }
        else if(gamepad1.dpad_down){
            program = Program.ONLY_MOVE;
        }

        telemetry.addData("Press X", "for BLUE");
        telemetry.addData("Press B", "for RED");
        telemetry.addData("Press Up", "for shoot + move program");
        telemetry.addData("Press Left", "for move, shoot, move program");
        telemetry.addData("Press Right", "for only shoot program");
        telemetry.addData("Selected Alliance", alliance);
        telemetry.addData("Selected Program", program);
    }
    ...
    public void loop() {
        ...
        switch(autonomousState){
            case INIT:
                if(program == Program.SHOOT_AND_MOVE){
                    autonomousState = AutonomousState.Program_ShootAndMove_Init;
                }
                else if(program == Program.MOVE_SHOOT_MOVE) {
                    autonomousState = AutonomousState.Program_MoveShootMove_Init;
                }
                else if(program == Program.ONLY_MOVE){
                    autonomousState = AutonomousState.Program_OnlyMove_Init;
                }
                else{
                    // WHAT DID YOU DO! THE CODE IS BROKEN!!!!!
                }
                break;
            ...
            case WAIT_FOR_LAUNCH:
                ...
                // ----------------------- Program Shoot and Move ----------------------
                case Program_ShootAndMove_Init:
                    nextState = AutonomousState.Program_ShootAndMove_DrivingAwayFromGoal;
                    autonomousState = AutonomousState.LAUNCH;
                    break;
                case Program_ShootAndMove_DrivingAwayFromGoal:
                    if(drive(DRIVE_SPEED, 10, DistanceUnit.INCH, 1)){
                        leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        autonomousState = AutonomousState.Program_ShootAndMove_RotatingAwayFromGoal;
                    }
                    break;
                case Program_ShootAndMove_RotatingAwayFromGoal:
                    robotRotationAngle = 110 * (alliance == Alliance.RED ? 1 : -1);
                    if(rotate(ROTATE_SPEED, robotRotationAngle, AngleUnit.DEGREES,1)){
                        leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        autonomousState = AutonomousState.Program_ShootAndMove_DrivingOffLine;
                    }
                    break;
                ...
                // ----------------------- Program Move Shoot Move ----------------------
                    
                case Program_MoveShootMove_Init:
                    nextState = AutonomousState.Program_MoveShootMove_DrivingToGoal;
                    waitTime = 7.0;
                    autonomousState = AutonomousState.DELAY;
                    break;
                case Program_MoveShootMove_DrivingToGoal:
                    if(drive(DRIVE_SPEED, -80, DistanceUnit.INCH, 1)){
                        leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        autonomousState = AutonomousState.Program_MoveShootMove_RotatingToGoal;
                    }
                    break;
                case Program_MoveShootMove_RotatingToGoal:
                    robotRotationAngle = 45 * (alliance == Alliance.RED ? -1 : 1);
                    if(rotate(ROTATE_SPEED, robotRotationAngle, AngleUnit.DEGREES,1)){
                        leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        autonomousState = AutonomousState.Program_MoveShootMove_DrivingToShootPosition;
                    }
                    break;
                case Program_MoveShootMove_DrivingToShootPosition:
                    if(drive(DRIVE_SPEED, -47, DistanceUnit.INCH, 1)){
                        leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        autonomousState = AutonomousState.Program_ShootAndMove_Init;
                    }
                ...
                case DELAY:
                    autoStopwatch.reset();
                    autonomousState = AutonomousState.DELAY_WAIT;
                    break;
                case DELAY_WAIT:
                    if(autoStopwatch.time() >= waitTime){
                        autonomousState = nextState;
                    }
                    break;
                case COMPLETE:
        ...
        switch (autonomousState){
            ...
            case INIT:
                if(program == Program.PUSH_BALLS_INTO_LOAD_ZONE){
                    autonomousState = AutonomousState.PUSHING_BALLS_INTO_TEAM_ZONE;
                }
                else {
                    autonomousState = AutonomousState.LAUNCH;
                }
            ...
            case ROTATE2:
                ...
                if(rotate(ROTATE_SPEED, robotRotationAngle, AngleUnit.DEGREES,1)){
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autonomousState = AutonomousState.DRIVING_TO_BALLS;
                }
                break;
            ...
            case PUSHING_BALLS_INTO_TEAM_ZONE:
                if(driveandturn(DRIVE_SPEED, -15, 0.5, DistanceUnit.INCH, 1)){
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autonomousState = AutonomousState.COMPLETE;
                }
     ...
     */
    boolean driveandturn(double speed, double distance, double Hamt, DistanceUnit distanceUnit, double holdSeconds) {
        ...
        leftDrive.setTargetPosition((int) (lAmt * targetPosition));
        rightDrive.setTargetPosition((int) (rAmt * targetPosition));

        leftDrive.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        rightDrive.setMode(DcMotor.RunMode.RUN_TO_POSITION);

        leftDrive.setPower(lAmt * 0.5);
        rightDrive.setPower(rAmt * 0.5);
